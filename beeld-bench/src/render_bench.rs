...
use beeld::vello_cpu::color::palette::css::WHITE;
...
const SUPPORTED_BACKENDS: [&str; 2] = ["pdfium", "beeld"];
const FAILURE_TEXT: &str = "failed to open PDF";
const TIME_EXAMPLE: &str = "12345.678 ms";
const DELTA_HEADER: &str = "beeld vs pdfium";
...
impl RenderBackend for BeeldRenderBackend {
    fn name(&self) -> &'static str {
        "beeld"
    }
    ...
    fn render_document(
        ...
        let render_settings = beeld::RenderSettings {
            bg_color: WHITE,
            ..Default::default()
        };
        let mut total_bytes = 0usize;
        ...
        for iteration in 0..iterations {
            let document = beeld::beeld_syntax::Pdf::new(Arc::clone(&pdf_bytes))
                .map_err(|err| format!("load failed: {err:?}"))?;
            let cache = beeld::RenderCache::new();

            for page in document.pages().iter() {
                let pixmap = beeld::render(page, &cache, &interpreter_settings, &render_settings);
        ...
        let duration = start.elapsed() / iterations as u32;
        let bitmaps = if collect_bitmaps {
            let document = beeld::beeld_syntax::Pdf::new(pdf_bytes)
                .map_err(|err| format!("load failed: {err:?}"))?;
            let cache = beeld::RenderCache::new();
            ...
            document
                ...
                .map(|(page_index, page)| {
                    let pixmap =
                        beeld::render(page, &cache, &interpreter_settings, &render_settings);
...
impl TableLayout {
    fn new(backends: &[Box<dyn RenderBackend>], pdfs: &[PathBuf], input_root: &Path) -> Self {
        ...
        let backend_widths = backend_names
            ...
            .collect::<Vec<_>>();
        let delta_width = if backend_names.contains(&"pdfium") && backend_names.contains(&"beeld") {
...
fn print_help(program: &OsString) {
    ...
    println!("Options:");
    println!(
        "  --backend <name>     Backend to use: pdfium, beeld, comma-separated list, or all. Default: pdfium,beeld"
    );
    println!(
...
fn create_backends(backend_names: &[String]) -> Result<Vec<Box<dyn RenderBackend>>, String> {
    ...
    for backend_name in backend_names {
        match backend_name.as_str() {
            ...
            "beeld" => backends.push(Box::new(BeeldRenderBackend::new())),
...
fn print_table_row(layout: &TableLayout, pdf_name: &Path, cells: Vec<BackendCell>) {
    ...
    for (backend_name, cell) in layout.backend_names.iter().copied().zip(cells) {
        match cell {
            BackendCell::Success(duration) => {
                ...
                if backend_name == "beeld" {
                    beeld_duration = Some(duration);
                }
    ...
    if let Some(width) = layout.delta_width {
        let delta = match (pdfium_duration, beeld_duration) {
            (Some(pdfium), Some(beeld)) => format_beeld_delta(pdfium, beeld),
            _ => String::from("-"),
        };
...
fn format_beeld_delta(pdfium: Duration, beeld: Duration) -> String {
    let pdfium_ms = pdfium.as_secs_f64() * 1000.0;
    if pdfium_ms == 0.0 {
        return String::from("-");
    }

    let beeld_ms = beeld.as_secs_f64() * 1000.0;
    let percent = ((beeld_ms - pdfium_ms) / pdfium_ms) * 100.0;
