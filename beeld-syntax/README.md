...
[![Crates.io](https://img.shields.io/crates/v/beeld-syntax.svg)](https://crates.io/crates/beeld-syntax)
[![Documentation](https://docs.rs/beeld-syntax/badge.svg)](https://docs.rs/beeld-syntax)
...
Such functionality is out-of-scope for `beeld-syntax`, since this crate is supposed to be
...
let data = std::fs::read(
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../beeld-tests/pdfs/custom/text_with_rise.pdf"),
)
.unwrap();
...
## Safety
When the `unsafe` feature is disabled, there is only use of `unsafe` in `beeld-syntax`.
