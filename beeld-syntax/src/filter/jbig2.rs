...
pub(crate) fn decode(
    ...
    let image = beeld_jbig2::Image::new_embedded(data, globals.as_deref()).ok()?;
    ...
    let (decoded, bpc) = if image_params.is_indexed {
        ...
        impl beeld_jbig2::Decoder for BitWriterDecoder<'_> {
        ...
        impl beeld_jbig2::Decoder for Luma8Decoder {
            fn push_pixel(&mut self, black: bool) {
                if black {
