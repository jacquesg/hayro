...
pub(crate) fn decode_bitmap_mmr(bitmap: &mut Bitmap, data: &[u8]) -> Result<usize> {
    ...
    impl beeld_ccitt::Decoder for BitmapDecoder<'_> {
        #[inline]
    ...
    let settings = beeld_ccitt::DecodeSettings {
        columns: width,
        rows: height,
        ...
        encoding: beeld_ccitt::EncodingMode::Group4,
        ...
        // (6.2.6)
        //
        // beeld-ccitt uses 1 for white, 0 for black by default, so we need to
    ...
    // read." (6.2.6)
    //
    // beeld-ccitt already aligns to the byte boundary before returning, so
    // nothing else to do here.
    let mut context = beeld_ccitt::DecoderContext::new(settings);
    Ok(beeld_ccitt::decode(data, &mut decoder, &mut context)
