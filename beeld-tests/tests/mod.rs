...
fn get_noto_fallback(query: &beeld::beeld_interpret::font::FallbackFontQuery) -> Option<FontData> {
...
fn render_pdf(
    ...
    beeld::render_pdf(pdf, 1.0, settings, range)
        .unwrap()
        .into_iter()
        .enumerate()
        .map(|(idx, d)| {
...
fn render_svg(
    ...
    let cache = beeld_svg::RenderCache::new();
    pdf.pages()
        .iter()
        .enumerate()
        .flat_map(|(idx, p)| {
            ...
            let svg = beeld_svg::convert(p, &cache, &interpreter_settings, &render_settings);
...
pub fn run_write_test(
    ...
    let beeld_pdf = load_pdf(file_path);

    let buf = if page {
        beeld_write::extract_pages_to_pdf(&beeld_pdf, page_indices)
    } else {
        beeld_write::extract_pages_as_xobject_to_pdf(&beeld_pdf, page_indices)
    };
...
fn get_standard(font: &StandardFont) -> Option<FontData> {
    match font {
        ...
        StandardFont::ZapfDingBats => Some(Arc::new(
            &include_bytes!("../../beeld-interpret/assets/FoxitDingbats.pfb")[..],
        )),
        StandardFont::Symbol => Some(Arc::new(
            &include_bytes!("../../beeld-interpret/assets/FoxitSymbol.pfb")[..],
        )),
    }
}
