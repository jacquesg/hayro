...
fn dont_cache_page_references() {
    let beeld_pdf = load_pdf("pdfs/custom/clip_path_evenodd.pdf");
    let mut next_ref = Ref::new(1);
    let extracted = beeld_write::extract(
        &beeld_pdf,
        Box::new(|| next_ref.bump()),
        beeld_write::ChunkSettings::default(),
        |_| {},
...
fn write_xobject_uses_isolated_transparency_group() {
    let beeld_pdf = load_pdf("pdfs/custom/clip_path_evenodd.pdf");
    let extracted = beeld_write::extract_pages_as_xobject_to_pdf(&beeld_pdf, &[0]);
    ...
    let group = x_object
        .dict()
        .get::<beeld_syntax::object::Dict<'_>>(GROUP)
        .unwrap();

    assert_eq!(
        group
            .get::<beeld_syntax::object::Name<'_>>("Type")
            .unwrap()
            .as_ref(),
        b"Group"
    );
    assert_eq!(
        group
            .get::<beeld_syntax::object::Name<'_>>("S")
            .unwrap()
            .as_ref(),
        b"Transparency"
    );
    ...
    assert_eq!(
        group
            .get::<beeld_syntax::object::Name<'_>>("CS")
            .unwrap()
            .as_ref(),
        b"DeviceRGB"
    );
...
fn write_null_objects() {
    let beeld_pdf = load_pdf("pdfs/other/issue188.pdf");
    ...
    let extracted = beeld_write::extract_pages_to_pdf(&beeld_pdf, &[0]);

    let reread = Pdf::new(extracted).unwrap();
    let dict = reread.pages()[0]
        .raw()
        .get::<beeld_syntax::object::Dict>("Resources")
        .unwrap()
        .get::<beeld_syntax::object::Dict>("Font")
        .unwrap();
    let data = dict.data();
