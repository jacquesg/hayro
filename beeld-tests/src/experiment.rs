...
use beeld_cmap::CMap;
use beeld_jpeg2000::DecodeSettings;
use beeld_syntax::Filter;
                ...
                for object in pdf.objects() {
                        ...
                        && stream.filters().first() == Some(&Filter::JpxDecode)
                    {
                        ...
                        let decoded = catch_unwind(|| {
                            beeld_jpeg2000::Image::new(&raw_data, &settings).and_then(|image| {
                                let mut decoder_context = beeld_jpeg2000::DecoderContext::default();
