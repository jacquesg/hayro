...
use beeld_syntax::object::dict::keys::{CHAR_PROCS, FONT_BBOX, FONT_MATRIX, RESOURCES};
use beeld_syntax::page::Resources;
...
impl<'a> Type3<'a> {
    pub(crate) fn new(dict: &Dict<'a>, cmap_resolver: &CMapResolverFn) -> Option<Self> {
        ...
        let font_bbox = dict
            .get::<beeld_syntax::object::Rect>(FONT_BBOX)
            .unwrap_or(beeld_syntax::object::Rect::ZERO)
            .to_kurbo();
