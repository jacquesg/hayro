...
use beeld_syntax::content::ops::TypedInstruction;
use beeld_syntax::object::dict::keys::{ACTUAL_TEXT, ANNOTS, AP, F, MCID, N, OC, RECT};
use beeld_syntax::object::{Array, Dict, Name, Object, Rect, Stream, String, dict_or_stream};
use beeld_syntax::page::{Page, Resources};
...
pub type CMapResolverFn =
    Arc<dyn Fn(beeld_cmap::CMapName<'_>) -> Option<&'static [u8]> + Send + Sync>;
...
pub struct InterpreterSettings {
    ...
    /// beeld can therefore read the font files and do all the processing needed. However, there
    /// are two problems:
    ...
    /// If you prefer smaller fonts, you can use the [Foxit CFF fonts](https://github.com/LaurenzV/beeld/tree/master/assets/standard_fonts),
    ...
    /// method from `beeld-cmap` by default, which embeds the cmap files for
    ...
    /// In certain cases, `beeld` will emit a warning in case an issue was encountered while interpreting
...
impl Default for InterpreterSettings {
    fn default() -> Self {
        Self {
            ...
            cmap_resolver: Arc::new(beeld_cmap::load_embedded),
